...
export default function QcFilterBar({ value, onChange, techIds }: { value: QcFilters; onChange: (value: QcFilters) => void; techIds: string[] }) {
  return <div className="qc-filter-bar">
    <label>Period<select value={value.month ? "month" : "all"} onChange={e => onChange({ ...value, month: e.target.value === "all" ? "" : fiscalMonthKey() })}><option value="all">All time</option><option value="month">Fiscal month</option></select></label>
    {value.month && <label>Month ending on the 21st<input type="month" value={value.month} onChange={e => onChange({ ...value, month: e.target.value })}/></label>}
