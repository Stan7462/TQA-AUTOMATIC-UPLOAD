...
export default function Profile(){
 ...
 async function prepareRedo(item:Submission,replaceDraft=false){
   ...
   try{
     ...
     await writeQcDraft({techId,submissionId:crypto.randomUUID(),jobNumber:/^\d{1,6}$/.test(item.jobNumber)?item.jobNumber:'',screenshot:pictures[0],photos:pictures.slice(1),updatedAt:Date.now()});
